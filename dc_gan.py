...
def train_discriminator(
    ...
    def loss_discriminator(
        ...
        z = jax.random.normal(key=key, shape=(len(x), 1, 1, args.nz))
        ...
        fake_labels = jnp.zeros(shape=(len(x),))
...
def train_generator(
    ...
    def loss_generator(
        ...
        """Loss of generator"""
        z = jax.random.normal(key=key, shape=(args.batch_size, 1, 1, args.nz))
    ...
    (loss_g, batch_stats_g), grads_g = grad_value_fn_g(state_g.params)
...
def train_step(
    ...
    state_d, loss_d = train_discriminator(
        x=x, state_g=state_g, state_d=state_d, key=key_d
    )

    state_g, loss_g = train_generator(state_g=state_g, state_d=state_d, key=key_g)
...
def train(
    ...
    for samples in tqdm(
        ...
        # state_d, loss_d = train_discriminator(
        #     x=x, state_g=state_g, state_d=state_d, key=key
        # )

        # state_g, loss_g = train_generator(state_g=state_g, state_d=state_d, key=key)
...
def initialise_generator(ngf: int, nz: int, key: jax.random.PRNGKey) -> TrainState:
    key1, key2 = jax.random.split(key=key, num=2)

    generator = Generator(ngf=ngf, nc=3)
    params = generator.init(
        rngs=key1,
        x=jax.random.normal(key=key2, shape=(1, 1, 1, nz)),
        # x=jnp.ones(shape=(1, 1, 1, nz), dtype=jnp.float32),
        train=False
    )
...
def initialise_discriminator(ndf: int, image_shape: tuple[int, int, int], key: jax.random.PRNGKey) -> TrainState:
    key1, key2 = jax.random.split(key=key, num=2)
    ...
    params = discriminator.init(
        ...
        # x=jnp.ones(shape=(1,) + image_shape, dtype=jnp.float32),
        train=False
    )
    tx = optax.adam(learning_rate=args.lr, b1=0.5)
    state = TrainState.create(
...
def main() -> None:
    ...
    # region MODELS
    args.key, key_g, key_d = jax.random.split(key=args.key, num=3)
    state_g = initialise_generator(ngf=args.ngf, nz=args.nz, key=key_g)
    state_d = initialise_discriminator(ndf=args.ndf, image_shape=dset_train[0]['image'].shape, key=key_d)
    # endregion
    ...
    with mlflow.start_run(
        ...
        for epoch_id in tqdm(
            ...
            mlflow.log_image(
                image=np.asarray(a=x_fake[0], dtype=np.float32),
                key='generated_image',
                step=epoch_id + 1
            )
